...
public class UIMatchDialog extends Dialog implements MatchDialog {
    ...
    private boolean mPopupStyle = false;
    ...
    public UIMatchDialog(Context context, boolean showSeparateLine) {
        ...
        mFullscreen = AppDisplay.getInstance(context).isPad() ? false : true;
        mMetrics = mContext.getResources().getDisplayMetrics();
        requestWindowFeature(Window.FEATURE_NO_TITLE);
        initView();
    }
    ...
    public void setButton(long buttons) {
        if (buttons == DIALOG_NO_BUTTON) {
            ...
            if ((buttons & MatchDialog.DIALOG_CANCEL) == MatchDialog.DIALOG_CANCEL) {
                ...
                addButton(mDlg_bt_cancel, mContext.getResources().getString(R.string.fx_string_cancel), MatchDialog.DIALOG_CANCEL);
            }
            if ((buttons & MatchDialog.DIALOG_SKIP) == MatchDialog.DIALOG_SKIP) {
                mDlg_bt_skip = new Button(mContext);
                addButton(mDlg_bt_skip, mContext.getResources().getString(R.string.fm_paste_skip), MatchDialog.DIALOG_SKIP);
            }
            if ((buttons & MatchDialog.DIALOG_REPLACE) == MatchDialog.DIALOG_REPLACE) {
                mDlg_bt_replace = new Button(mContext);
                addButton(mDlg_bt_replace, mContext.getResources().getString(R.string.fm_paste_replace), MatchDialog.DIALOG_REPLACE);
            }
            if ((buttons & MatchDialog.DIALOG_COPY) == MatchDialog.DIALOG_COPY) {
                mDlg_bt_copy = new Button(mContext);
                addButton(mDlg_bt_copy, mContext.getResources().getString(R.string.fx_string_copy), MatchDialog.DIALOG_COPY);
            }
            if ((buttons & MatchDialog.DIALOG_MOVE) == MatchDialog.DIALOG_MOVE) {
                mDlg_bt_move = new Button(mContext);
                addButton(mDlg_bt_move, mContext.getResources().getString(R.string.fm_move), MatchDialog.DIALOG_MOVE);
            }
            if ((buttons & MatchDialog.DIALOG_OK) == MatchDialog.DIALOG_OK) {
                mDlg_bt_ok = new Button(mContext);
                addButton(mDlg_bt_ok, mContext.getResources().getString(R.string.fx_string_ok), MatchDialog.DIALOG_OK);
            }
            if ((buttons & MatchDialog.DIALOG_OPEN_ONLY) == MatchDialog.DIALOG_OPEN_ONLY) {
                mDlg_bt_open_only = new Button(mContext);
                addButton(mDlg_bt_open_only, mContext.getResources().getString(R.string.rv_emailreview_mergedlg_openbutton), MatchDialog.DIALOG_OPEN_ONLY);
            }
            if ((buttons & MatchDialog.DIALOG_UPLOAD) == MatchDialog.DIALOG_UPLOAD) {
                mDlg_bt_upload = new Button(mContext);
                addButton(mDlg_bt_upload, mContext.getResources().getString(R.string.cloud_toolbar_more_upload), MatchDialog.DIALOG_UPLOAD);
            }
