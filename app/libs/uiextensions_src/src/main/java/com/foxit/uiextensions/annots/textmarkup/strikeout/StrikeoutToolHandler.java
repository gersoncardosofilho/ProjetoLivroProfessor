...
public class StrikeoutToolHandler implements ToolHandler {
    ...
    public boolean OnSelectRelease(int pageIndex, SelectInfo selectInfo, boolean addUndo, Event.Callback result) {
        if (selectInfo == null) return false;
        ...
        addAnnot(pageIndex, addUndo, mSelectInfo.mRectArray, rectF, selectInfo, result);
    ...
    private void addAnnot(final int pageIndex, final boolean addUndo, final ArrayList<RectF> rectArray, final RectF rectF, final SelectInfo selectInfo, final Event.Callback result) {
        ...
        undoItem.mType = Annot.e_annotStrikeOut;
    ...
    private String getContent(PDFPage page, SelectInfo selectInfo) {
        ...
        try {
            ...
            textPage = new PDFTextSelect(page);
    ...
    public void AddAnnot(final int pageIndex, final boolean addUndo, AnnotContent contentSupplier, ArrayList<RectF> rectFs, final RectF dmRectf,
        ...
        try {
            final StrikeOut annot = (StrikeOut) mPdfViewCtrl.getDoc().getPage(pageIndex).addAnnot(Annot.e_annotStrikeOut, dmRectf);
            ...
            for (int i = 0; i < pointsList.size() / 4; i++) {
                undoItem.mQuadPoints[i] = new QuadPoints(pointsList.get(4 * i), pointsList.get(4 * i + 1),
