...
//                            mAdapter.updateAnnotationListFromCheck();
    ...
    void prepareDeleteNodeList() {
        ...
        for (AnnotNode node : mCheckedNodes) {
            if (node.getParent() != null && mCheckedNodes.contains(node.getParent()))
                continue;
    ...
    private DocumentManager.AnnotEventListener mAnnotEventListener = new DocumentManager.AnnotEventListener() {
        ...
        public void onAnnotAdded(PDFPage page, Annot annot) {
            ...
            try {
                if (!AppAnnotUtil.isSupportEditAnnot(annot) || (annot.getFlags() & Annot.e_annotFlagHidden) != 0 || !isPageLoaded(page))
                    return;
            ...
            try {
                ...
                if (nCount > 0) {
                    for (int i = 0; i < nCount; i++) {
                        ...
                        AnnotHandler annotHandler = ToolUtil.getAnnotHandlerByType(uiExtensionsManager, annot.getType());
                        if (annot == null || null == annotHandler
                                || (annot.getFlags() & Annot.e_annotFlagHidden) != 0 || !AppAnnotUtil.isSupportEditAnnot(annot))
                            continue;

                        if (annotHandler instanceof CaretAnnotHandler || annotHandler instanceof LineAnnotHandler){
                            ToolHandler toolHandler = uiExtensionsManager.getToolHandlerByType(AppAnnotUtil.getTypeToolName(annot));
                            if (toolHandler == null){
                                continue;
                            }
                        }

                        if (annotHandler instanceof InkAnnotHandler
                                && !uiExtensionsManager.getModulesConfig().getAnnotConfig().isLoadDrawPencil()){
                            continue;
                        }
