...
public class DocInfoView {
    ...
    class SummaryInfo extends DocInfo {
        ...
        DocumentInfo getDocumentInfo() {
            ...
            try {
                PDFMetadata metadata = new PDFMetadata(doc);
                info.mAuthor = metadata.getValue("Author");
                info.mSubject = metadata.getValue("Subject");
    ...
    class PermissionInfo extends DocInfo {
        ...
        void init() {
            ...
            tvFillForm.setText(DocumentManager.getInstance(mPdfViewCtrl).canFillForm() ? allowed : notAllowed);
            tvAnnotForm.setText(DocumentManager.getInstance(mPdfViewCtrl).canAddAnnot() ? allowed : notAllowed);
            tvAssemble.setText(DocumentManager.getInstance(mPdfViewCtrl).canAssemble() ? allowed : notAllowed);
            tvModify.setText(DocumentManager.getInstance(mPdfViewCtrl).canModifyContents() ? allowed : notAllowed);
            tvExtractAccess.setText(DocumentManager.getInstance(mPdfViewCtrl).canCopyForAssess() ? allowed : notAllowed);
            tvExtract.setText(DocumentManager.getInstance(mPdfViewCtrl).canCopy()? allowed : notAllowed);
            boolean canSign = DocumentManager.getInstance(mPdfViewCtrl).canSigning();
