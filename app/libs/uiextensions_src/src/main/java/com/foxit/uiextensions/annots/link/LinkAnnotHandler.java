...
import com.foxit.sdk.pdf.action.LaunchAction;
import com.foxit.sdk.pdf.action.RemoteGotoAction;
...
//        mPaint.setARGB(0x16, 0x0, 0x7F, 0xFF);
    ...
    protected PointF getDestinationPoint(Destination destination) {
        if (destination == null) {
            return null;
        }
...
//        if (pageIndex != mPdfViewCtrl.getCurrentPage()) return false;
        if (!mPdfViewCtrl.isPageVisible(pageIndex)) return false;

        UIExtensionsManager.ILinkEventListener ILinkEventListener = ((UIExtensionsManager) mPdfViewCtrl.getUIExtensionsManager()).getLinkEventListener();
        ...
        try {
            ...
            switch (type) {
                ...
                case Action.e_actionTypeURI: {
                    if (mPdfViewCtrl.getUIExtensionsManager() == null) return true;
                ...
                case Action.e_actionTypeLaunch:
                case Action.e_actionTypeGoToR: {
                    FileSpec fileSpec;
                    if (annotAction instanceof RemoteGotoAction) {
                        RemoteGotoAction remoteGotoAction = (RemoteGotoAction) annotAction;
                        fileSpec = remoteGotoAction.getFileSpec();
                    } else {
                        LaunchAction remoteGotoAction = (LaunchAction) annotAction;
                        fileSpec = remoteGotoAction.getFileSpec();
                    }
                    ...
                    if (path != null && path.lastIndexOf("/") > 0) {
                        ...
                        Context context = ((UIExtensionsManager)mPdfViewCtrl.getUIExtensionsManager()).getAttachedActivity();
                        if (context == null) return false;
                        AppUtil.Alert((Activity) context,
                                AppResource.getString(mContext, R.string.annot_link_alert_title),
                                AppResource.getString(mContext, R.string.annot_link_alert_prompt),
                                AppUtil.ALERT_OK);
                    }
                    return true;
                }
