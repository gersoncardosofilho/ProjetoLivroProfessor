...
//    private boolean mSinglePage = true;//true:SINGLE_PAGE,false:CONTINUOUS_PAGE
    private int mFlag = PDFViewCtrl.PAGELAYOUTMODE_SINGLE;
    ...
    protected void init(PDFReader read) {
        mReader = read;
        ...
        mMoreToolsBar = new MoreToolsImpl(mContext, mRootView, mModuleConfig);
    ...
    private void initBottomBarBtns() {
        ...
        mPanelBtn.setTag(ToolbarItemConfig.ITEM_BOTTOMBAR_LIST_TAG);
        ...
        mSettingBtn.setTag(ToolbarItemConfig.ITEM_BOTTOMBAR_VIEW_TAG);
        ...
        if (mModuleConfig.isLoadAnnotations()) {
            ...
            mEditBtn.setTag(ToolbarItemConfig.ITEM_BOTTOMBAR_COMMENT_TAG);
        ...
        if (mModuleConfig.isLoadSignature()) {
            ...
            mReadSignItem.setTag(ToolbarItemConfig.ITEM_BOTTOMBAR_SIGN_TAG);
    ...
    protected void resetAnnotCustomBottomBar() {
        mAnnotCustomBottomBar.removeAllItems();
        mAnnotCustomBottomBar.setBackgroundColor(mContext.getResources().getColor(com.foxit.uiextensions.R.color.ux_bg_color_toolbar_light));
        mAnnotCustomBottomBar.setItemSpace(mContext.getResources().getDimensionPixelSize(com.foxit.uiextensions.R.dimen.rd_bottombar_button_space));
        ...
        mAnnotCustomBottomBar.addView(mSignListItem, BaseBar.TB_Position.Position_CENTER);
    }

    protected void resetAnnotCustomTopBar() {
        mAnnotCustomTopBar.removeAllItems();
        mAnnotCustomTopBar.setBackgroundColor(mContext.getResources().getColor(com.foxit.uiextensions.R.color.ux_bg_color_toolbar_light));
        ...
        mAnnotCustomTopBar.addView(closeItem, BaseBar.TB_Position.Position_LT);
        mAnnotCustomTopBar.addView(titleItem, BaseBar.TB_Position.Position_LT);
    }
    ...
    private void initOtherView() {
        ...
        mBackItem.setTag(ToolbarItemConfig.ITEM_TOPBAR_BACK_TAG);
        mTopBar.addView(mBackItem, BaseBar.TB_Position.Position_LT);
        mBackItem.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ((UIExtensionsManager) mReader.getUIExtensionsManager()).triggerDismissMenuEvent();
                if (mReader.getBackEventListener() != null && mReader.getBackEventListener().onBack()) {
                    return;
                }
    ...
    protected void addDocView(View docView) {
    ...
    private void applyValue() {
        if (mFlag == PDFViewCtrl.PAGELAYOUTMODE_SINGLE) {
            mReader.getDocViewer().setPageLayoutMode(PDFViewCtrl.PAGELAYOUTMODE_SINGLE);
        } else if (mFlag == PDFViewCtrl.PAGELAYOUTMODE_CONTINUOUS){
            mReader.getDocViewer().setPageLayoutMode(PDFViewCtrl.PAGELAYOUTMODE_CONTINUOUS);
        } else if (mFlag == PDFViewCtrl.PAGELAYOUTMODE_FACING){
            mReader.getDocViewer().setPageLayoutMode(PDFViewCtrl.PAGELAYOUTMODE_FACING);
        }
    }

    IMultiLineBar.IML_ValueChangeListener mSingleChangeListener = new IMultiLineBar.IML_ValueChangeListener() {
        @Override
        public void onValueChanged(int type, Object value) {
            if (IMultiLineBar.TYPE_SINGLEPAGE == type) {
                mFlag = (Integer)value;
                mSettingBar.setProperty(IMultiLineBar.TYPE_SINGLEPAGE, mFlag);
                applyValue();
            }
        }
        ...
        @Override
        public int getType() {
            return IMultiLineBar.TYPE_SINGLEPAGE;
        }
    };
    ...
    IMultiLineBar.IML_ValueChangeListener mThumbnailListener = new IMultiLineBar.IML_ValueChangeListener() {
        @Override
        public void onValueChanged(int type, Object value) {
            if (IMultiLineBar.TYPE_THUMBNAIL == type) {
                showThumbnailDialog();
        ...
        @Override
        public int getType() {
            return IMultiLineBar.TYPE_THUMBNAIL;
        }
    };
