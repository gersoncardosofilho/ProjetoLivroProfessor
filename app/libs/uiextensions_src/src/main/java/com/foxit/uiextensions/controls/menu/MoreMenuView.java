    ...
    public void addDocInfoItem() {
        ...
        MenuItemImpl rfsItem = new MenuItemImpl(mContext, MoreMenuConfig.ITEM_REDUCE_FILE_SIZE,
                ...
                new MenuViewCallback() {
                    @Override
                    public void onClick(MenuItemImpl item) {
                        PDFReader pdfReader = (PDFReader) ((UIExtensionsManager)mPdfViewCtrl.getUIExtensionsManager()).getPDFReader();
                        if (pdfReader != null) {
                            pdfReader.setSaveDocFlag(PDFDoc.e_saveFlagXRefStream);
                        }
