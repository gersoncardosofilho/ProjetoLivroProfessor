...
class EraserToolHandler extends AbstractToolHandler {
        ...
        if (mRootList.size() > 1) {
            for (int k = 0; k < mRootList.size(); k++) {
                ...
                if (!annotInfo.mModifyFlag) {
                    ...
                    try {
                        if (!hasNoModify) {
                            noModifyRect.set(annotInfo.mAnnot.getRect());
                        } else {
                            union(noModifyRect, annotInfo.mAnnot.getRect());
                        }
        ...
        if (!mRootList.isEmpty()) {
            ...
            try {
                ...
                for (int i = mRootList.size() - 1; i >= 0; i--) {
                    ...
                    invalidateJniAnnots(annotInfo, 1, null);
                    union(rmRect, annotInfo.mAnnot.getRect());
                }
    ...
    private void invalidateJniAnnots(AnnotInfo annotInfo, int flag, final Event.Callback result) {
        ...
        if (flag == 0) {
            ...
            DocumentManager.getInstance(mPdfViewCtrl).onAnnotEndEraser(annotInfo.mAnnot);
        }
    ...
    private void modifyAnnot(final int pageIndex, final AnnotInfo annotInfo, final EraserUndoItem undoItems, final boolean isLast) {
        try {
            ...
            InkAnnotHandler annotHandler = (InkAnnotHandler) ToolUtil.getAnnotHandlerByType((UIExtensionsManager) mPdfViewCtrl.getUIExtensionsManager(), annot.getType());
            ...
            DocumentManager.getInstance(mPdfViewCtrl).setHasModifyTask(isLast);
                ...
                public void result(Event event, boolean success) {
                    if (success) {
                        try {
                            ...
                            if (isLast) {
                                DocumentManager.getInstance(mPdfViewCtrl).addUndoItem(undoItems);
                            ...
                            if (mPdfViewCtrl.isPageVisible(pageIndex)) {
                                ...
                                mPdfViewCtrl.refresh(pageIndex, rectFToRect(annotRectF));
