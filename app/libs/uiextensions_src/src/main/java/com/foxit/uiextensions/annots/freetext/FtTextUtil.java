...
public class FtTextUtil {
    private final float NEWLINE_BORDER_REDUNDANT_THRESHOLDS = 10f;
    ...
    private void initFontNameMap() {
        try {
            mFontNameMap = new HashMap<String, String>();
            Font Courier = new Font(Font.e_fontStandardIDCourier);
            mFontNameMap.put(Courier.getName(), "Courier");
            Courier.release();
            Font Helvetica = new Font(Font.e_fontStandardIDHelvetica);
            mFontNameMap.put(Helvetica.getName(), "Helvetica");
            Helvetica.release();
            Font Times = new Font(Font.e_fontStandardIDTimes);
            mFontNameMap.put(Times.getName(), "Times");
            Times.release();
    ...
    public String getSupportFontName(String key) {
        return mFontNameMap.get(key) != null ? mFontNameMap.get(key) : "Courier";
    }
    ...
    public Font getSupportFont(String name) {
        ...
        try {
            if (name == null) {
                font = new Font(Font.e_fontStandardIDCourier);
            } else if (name.equals("Courier")) {
                font = new Font(Font.e_fontStandardIDCourier);
            } else if (name.equals("Helvetica")) {
                font = new Font(Font.e_fontStandardIDHelvetica);
            } else if (name.equals("Times")) {
                font = new Font(Font.e_fontStandardIDTimes);
    ...
    public void loadText() {
        ...
        if (mTextContent != null && !mTextContent.equals("")) {
            ...
            if (mCurrentPageIndex == mPdfViewCtrl.getPageCount() - 1 || mPdfViewCtrl.getPageLayoutMode() == PDFViewCtrl.PAGELAYOUTMODE_SINGLE) {
    ...
    public ArrayList<String> getComposedText(PDFViewCtrl pdfViewCtrl, int pageIndex, RectF annotRect, String textContent, String fontName, float fontSize) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (ch == '\n' || ch == '\r') {
                ...
                if (tCurrentLineWidth > (((rotate == 90) || (rotate == 270)) ? (Math.abs(rect.height()) * 100 + NEWLINE_BORDER_REDUNDANT_THRESHOLDS) : (Math.abs(rect.width())) * 100 + NEWLINE_BORDER_REDUNDANT_THRESHOLDS)
                        && (((rotate == 90) || (rotate == 270)) ? (Math.abs(rect.height()) * 100 + NEWLINE_BORDER_REDUNDANT_THRESHOLDS) : (Math.abs(rect.width())) * 100 + NEWLINE_BORDER_REDUNDANT_THRESHOLDS) > chWidth) {
